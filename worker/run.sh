...
(sleep 10; sudo -u sgeuser bash -c "ssh ${SGEMASTER_PORT_22_TCP_ADDR} \"sudo bash -c '. /etc/profile.d/sge.sh; qconf -ah ${HOST_NAME:=${HOSTNAME}}; qconf -as ${HOST_NAME:=${HOSTNAME}}'\""; cd /opt/sge; ./inst_sge -x -auto /tmp/install_sge_worker.conf -nobincheck -noremote) &
